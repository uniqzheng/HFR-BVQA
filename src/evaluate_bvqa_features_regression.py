...
def evaluate_bvqa_one_split(i, X, y, num_cont, num_dists, log_short):
  ...
  if X_train.shape[1] <= 1100:
    ...
    #grid = RandomizedSearchCV(RandomForestRegressor(), param_grid, n_jobs=4, cv=pdsplit, verbose=0)
  else:
  ...
  #VT = VarianceThreshold(threshold = 0.05) #0.07--100, 0.08--40
  #VT.fit(X_train)
  ...
  #selector = RFE(estimator, n_features_to_select = 60, step = 150)
  #selector = RFECV(estimator, min_features_to_select = 60, step =10, cv =pdsplit) 
  ...
  #selector = SelectFromModel(LogisticRegression()).fit(X_train,y_train.astype('int'))
  #X_train = selector.transform(X_train)
  #X_test = selector.transform(X_test)
  ...
  # compute metrics
  y_train_pred = [(x+y)/2  for x, y in zip(y_train_pred_s,y_train_pred_t)]
  y_test_pred = [(x+y)/2   for x, y in zip(y_test_s,y_test_t)]
